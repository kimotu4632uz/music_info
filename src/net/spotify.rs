...
impl Spotify {
    pub fn new<P: AsRef<Path>>(cred: P, token: P) -> anyhow::Result<Spotify> {
        let client = Client::new(cred, token, "https://accounts.spotify.com/api/token".to_string())?;
        Ok(Spotify { client: client })
    }
}
...
impl FetchMeta for Spotify {
    fn query(&self, query: &str) -> anyhow::Result<Vec<(Metadata, AddInfo)>> {
        ...
        if resp["albums"]["total"].as_i64().unwrap_or(0) > 0 {
            let ids = resp["albums"]["items"].as_array().unwrap().iter().map(|v| v["id"].as_str().unwrap().to_string());
    ...
    fn fetch_all(&self, id: &str) -> anyhow::Result<(Metadata, AddInfo)> {
        let mut resp: inner_structs::Album = self.client
        ...
        for track in tracks_from {
            ...
            let artist = track.artists.iter().map(|x| x.name.as_str()).collect::<Vec<_>>().join(", ");

            tracks.push(Track::new(title, artist));
        }

        let ext_ids = resp.external_ids.iter().map(|(k, v)| (k.clone(), format!("{}", v)));

        let add_info = if !resp.images.is_empty() {
            let image = resp.images.iter().map(|x| format!("{} x {}", x.height, x.width)).collect::<Vec<_>>().join(", ");

            vec![("image".to_string(), image)].into_iter().chain(ext_ids).collect()
        } else {
            ext_ids.collect()
        };

        Ok((Metadata::new(Some(id.to_string()), album, date, genre, tracks), add_info))
    }
}
...
impl FetchPicture for Spotify {
    fn fetch_picture(&self, id: &str) -> anyhow::Result<Picture> {
        ...
        let url = resp.images.last().ok_or(anyhow::anyhow!("no cover art found"))?.url.as_str();
