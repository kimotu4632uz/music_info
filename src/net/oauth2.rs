...
use oauth2::{AuthUrl, TokenUrl, ClientId, ClientSecret, StandardTokenResponse, EmptyExtraTokenFields, TokenResponse};
...
fn get_timestamp() -> Duration {
    SystemTime::now().duration_since(UNIX_EPOCH).expect("your system or rust std::time is broken")
}

fn token_expired(token: &Token) -> bool {
    token.expires_in().map(|d| get_timestamp() > d).unwrap_or(false)
}
...
impl ClientCredential {
    pub fn perform_auth<P: AsRef<Path>>(&self, auth_url: &str, token_store: P) -> anyhow::Result<Token> {
        let client = BasicClient::new(ClientId::new(self.client_id.clone()), Some(ClientSecret::new(self.client_secret.clone())), AuthUrl::new(auth_url.to_string()).unwrap(), Some(TokenUrl::new(auth_url.to_string()).unwrap()));
        ...
        std::fs::write(token_store, serde_json::to_string_pretty(&token_resp).unwrap())?;
...
impl Client {
    pub fn new<P: AsRef<Path>>(client_cred: P, access_token: P, auth_url: String) -> anyhow::Result<Client> {
        let client_cred_path = client_cred.as_ref();
        let access_token_path = access_token.as_ref();

        if !client_cred_path.exists() { anyhow::bail!("client credential file not exists.") }
        ...
        Ok(Client{cred: cred, token: RefCell::new(token), auth_url: auth_url, token_store: access_token_path.into()})
    }
    ...
    pub fn get(&self, url: &str) -> anyhow::Result<ureq::Request> {
        ...
        let token = self.token.borrow();
        let auth = format!("{} {}", tt2str(token.token_type()), token.access_token().secret());
